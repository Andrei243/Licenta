...
Matrix& Matrix::SetTranslation(GLfloat x, GLfloat y, GLfloat z, TypeOfSpace type)
{
	GLfloat length, factor;
	GLfloat c1, c2;
	Matrix mat, mat2;
	...
	mat.m[0][0] = 0.0; mat.m[0][1] = 0.0; mat.m[0][2] = 0.0; mat.m[0][3] = 0.0;
	mat.m[1][0] = 0.0; mat.m[1][1] = 0.0; mat.m[1][2] = 0.0; mat.m[1][3] = 0.0;
	mat.m[2][0] = 0.0; mat.m[2][1] = 0.0; mat.m[2][2] = 0.0; mat.m[2][3] = 0.0;
	mat.m[3][0] = x;   mat.m[3][1] = y;   mat.m[3][2] = z;   mat.m[3][3] = 0.0;
	switch (type) {
	case Spherical:
		mat.m[0][3] -= x;
		mat.m[1][3] -= y;
...
Matrix& Matrix::SetTranslation(GLfloat* pTrans,TypeOfSpace type)
{
	SetTranslation(pTrans[0], pTrans[1], pTrans[2], type);
	return *this;
}

Matrix& Matrix::SetTranslation(Vector3& vec,TypeOfSpace type)
{
	SetTranslation(vec.x, vec.y, vec.z, type);

	return *this;
}

Matrix& Matrix::SetPerspective(GLfloat fovY, GLfloat aspect, GLfloat nearPlane, GLfloat farPlane,TypeOfSpace type)
{
	...
	GLfloat halfWidth, halfHeight, c, n, f, a, b;
	halfWidth = 0.5 * width;
	halfHeight = 0.5 * height;
	c = 0.5 * (width >= height ? width : height);
	n = nearPlane;
	switch (type) {
	case Flat:
		f = 0.0;
